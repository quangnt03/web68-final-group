...
const AccountInfo = (props) => {
  ...
  const handleFormSubmit = async (data) => {
    ...
    // Kiểm tra password
    const response = await fetch('/account/check-password', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      const updateResponse = await fetch('/account/update-user', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const passwordChangFormSubmit = async (data) => {
    ...
    if (response.ok) {
      ...
      const updateResponse = await fetch('http://localhost:5000/account/change-password', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
